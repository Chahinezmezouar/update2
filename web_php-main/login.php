...
if (isset($_POST['fournisseur'])) {
	...
	// insert into BDD
	$sql = "INSERT INTO fournisseur (nom_f, prenom_f, email_f, mot_passe_f, wilaya_f) VALUES ('$nom_f', '$prenom_f', '$mail_f', '$pwd_hash2', '$wilaya_f')";
	if (mysqli_query($lien, $sql)) {
		// redirection
		session_start(); 
		$sessionid = $_SESSION['malak'];
...
$mail_f_enc = urlencode($mail_f);

// Redirection vers le deuxième fichier PHP en incluant les données encodées dans l'URL
header("Location: fourn2.php?nom_f=$nom_f_enc&mail_f=$mail_f_enc");
		exit();
	} else {
		echo "Erreur: " . $sql . "<br>" . mysqli_error($lien);
